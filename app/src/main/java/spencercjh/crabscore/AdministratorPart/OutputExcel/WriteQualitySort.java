...
class WriteQualitySort {
    ...
    static boolean writeExcel(ArrayList<GroupOBJ> AllGroup, WritableWorkbook wwb, WritableSheet sheet) throws Exception {
        Collections.sort(AllGroup, new Comparator<GroupOBJ>() {
            ...
            public int compare(GroupOBJ g1, GroupOBJ g2) {
                ...
                float quality_score_g2 = (g2.getQuality_score_m() + g2.getQuality_score_m() / (float) 2.0);
        ...
        String[] title = {"组号", "单位名称", "种质分", "排名"};
        Label label = new Label(0, 0, title[0], setBackGround_SkyBlue());
        sheet.addCell(label);
        label = new Label(0, 1, title[1], setBackGround_SkyBlue());
        sheet.addCell(label);
        label = new Label(0, 2, title[2], setBackGround_SkyBlue());
        sheet.addCell(label);
        label = new Label(0, 3, title[3], setBackGround_SkyBlue());
        sheet.addCell(label);
        for (int i = 0; i < AllGroup.size(); i++) {
            ...
            sheet.addCell(label);
            label = new Label(2, i + 1, String.valueOf((groupOBJ.getQuality_score_m() + groupOBJ.getQuality_score_f()) / (float) 2.0));
            sheet.addCell(label);
    ...
    private static WritableCellFormat setBackGround_SkyBlue() {
        ...
        try {
            ...
            format.setBackground(Colour.SKY_BLUE);// 天蓝色色背景

    ...
    static boolean writeExcel(ArrayList<GroupOBJ> AllGroup, WritableWorkbook wwb, WritableSheet sheet) throws Exception {
        Collections.sort(AllGroup, new Comparator<GroupOBJ>() {
            ...
            public int compare(GroupOBJ g1, GroupOBJ g2) {
                ...
                float taste_score_g2 = (g2.getTaste_score_m() + g2.getTaste_score_m() / (float) 2.0);
        ...
        sheet.addCell(label);
        label = new Label(0, 3, title[3], setBackGround_DarkPurple());
        sheet.addCell(label);
        for (int i = 0; i < AllGroup.size(); i++) {
            ...
            sheet.addCell(label);
            label = new Label(2, i + 1, String.valueOf((groupOBJ.getTaste_score_m() + groupOBJ.getTaste_score_f()) / (float) 2.0));
            sheet.addCell(label);

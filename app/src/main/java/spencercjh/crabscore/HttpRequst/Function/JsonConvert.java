...
public class JsonConvert {
    //    大赛切换中的大赛列表：年份-备注
    public static ArrayList<Competition_Info_OBJ> convert_year_note(String jsonstr) {
        ArrayList<Competition_Info_OBJ> list = new ArrayList<>();
        try {
            JSONArray jsonArray = new JSONArray(jsonstr);
            for (int i = 0; i < jsonArray.length(); i++) {
                JSONObject list_item = jsonArray.getJSONObject(i);
                int competition_year = list_item.getInt("competition_year");
                String note = list_item.getString("note");
                Competition_Info_OBJ competition_info_obj = new Competition_Info_OBJ(competition_year, note);
    ...
    public static ArrayList<UserOBJ> convert_user_name_display_name_role_id_status(String jsonstr) {
        ...
        try {
            ...
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                UserOBJ userOBJ = new UserOBJ(user_name, display_name, role_id, status, email);
                list.add(userOBJ);
            }
        } catch (JSONException e) {
            e.printStackTrace();
    ...
    public static ArrayList<Competition_Info_OBJ> convert_competition_property(String jsonstr) {
        ArrayList<Competition_Info_OBJ> list = new ArrayList<>();
        try {
            JSONArray jsonArray = new JSONArray(jsonstr);
            for (int i = 0; i < jsonArray.length(); i++) {
                ...
                Competition_Info_OBJ competition_info_obj = new Competition_Info_OBJ(competition_year,
